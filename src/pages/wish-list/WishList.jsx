...
const WishList = () => {
    ...
    const expiredDeals = totalDeals?.data?.filter(deal => new Date(deal?.promotedUntil) < now);
    const availableDeals = totalDeals?.data?.filter(deal => new Date(deal?.promotedUntil) >= now);
    ...
    return (
        <div className="bg-white px-4 pt-36 pb-12.5">
            <TabSection
                activeTab={activeTab}
                    ...
                    <div className="max-w-305 mx-auto pt-5">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                            {availableDeals?.map((deal, index) => (
                                <WishListCard key={index} deal={deal} />
                            ))}
                            {
