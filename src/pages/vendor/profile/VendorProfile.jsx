        ...
        <div className="bg-white min-h-[70vh] px-4 pt-32 pb-12">
            <div className="max-w-305 mx-auto space-y-6">
                    ...
                    <div className="flex justify-center md:justify-between gap-2">
                        <div className="flex flex-col md:flex-row items-center gap-8">
                            <div className="relative">
                                <div className="w-28 h-28 rounded-full border-4 border-white shadow-md overflow-hidden ring-4 ring-[#28B8D7]/20">
                                    <div className="w-full h-full bg-[#28B8D7] flex items-center justify-center text-white text-3xl font-bold">
                                        {shortName}
                                    </div>
                                </div>
                            </div>
                            <div className="flex-1">
                                <div className="text-center md:text-left space-y-3">
                                    <div className="flex flex-wrap items-center justify-center md:justify-start gap-3">
                                        ...
                                        <span className={`flex items-center gap-1 text-sm font-bold  px-4 py-1 rounded-full border border-[#28B8D7]/20 ${isVerified ? 'text-[#28B8D7] bg-[#28B8D7]/10' : 'text-[#d7c528] bg-amber-100'}`}>
                                ...
                                <div className="w-full md:hidden mt-5 flex flex-col items-center">
                                    <Link to='/show-outlet' className="w-full">
                                        <button className="mt-1 flex gap-1 items-center bg-[#4BBDCF] text-white w-full px-10 py-2.5 rounded-full text-base font-medium hover:bg-[#3db0c1] transition cursor-pointer">
                                            <Eye size={20} /> Show outlets
                                        </button>
                                    </Link>
                                    <Link to={`/verdor-edit-shop/${_id}`} className="w-full">
                                        <button className="mt-2 flex items-center gap-1 bg-[#4BBDCF] text-white w-full px-10 py-2.5 rounded-full text-base font-medium hover:bg-[#3db0c1] transition cursor-pointer">
                        ...
                        <div className="hidden md:block">
                            <Link to={`/show-outlet/${_id}`}>
                                <button className="mt-1 flex items-center bg-[#4BBDCF] text-white w-full px-10 py-2.5 rounded-full text-base gap-1 font-medium hover:bg-[#3db0c1] transition cursor-pointer">
                                    <Eye size={20} /> Show outlets
                                </button>
                            </Link>
                ...
                <div className="flex p-1 rounded-xl border border-slate-200 shadow-sm overflow-hidden gap-2">
                    <button className={`flex-1 py-3.5 px-6 rounded-lg text-base font-bold transition-all cursor-pointer ${activeTab == 'profile-details' ? 'bg-[#28B8D7] text-white hover:bg-[#4fb5cc]' : 'bg-slate-50 text-slate-500'}`} onClick={() => setActiveTab('profile-details')}>
                        Edit Profile Details
                    </button>
                    <button className={`flex-1 py-3.5 px-6 rounded-lg text-base font-bold transition-all cursor-pointer ${activeTab == 'password-change' ? 'bg-[#28B8D7] text-white hover:bg-[#4fb5cc]' : 'bg-slate-50 text-slate-500'}`} onClick={() => setActiveTab('password-change')}>
                        Change Password
                    </button>
                </div>
                {
